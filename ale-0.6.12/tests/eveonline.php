<?php
require_once '../ale/eveonline.php';
...
class AleEVEOnlineTest extends PHPUnit_Framework_TestCase
{
	protected function mockCache()
	{
		return $this->getMock("AleInterfaceCache", array('__construct', 'setHost', 'setCall', 'store', 'updateCachedUntil', 'retrieve', 'isCached', 'purge'));//, $arguments)
	}
	...
	{
		return $this->getMock("AleInterfaceRequest", array('__construct', 'query'));//, $arguments)
	}
	...
	public function testConstructorConfig()
	{
		...
		$eveonline = new AleEVEOnline($request, $cache, $config);
	...
	{
		$expected = file_get_contents('data/ServerStatus.xml.aspx');
	...
	{
		$expected = file_get_contents('data/CharacterName.xml.aspx');
	...
	{
		$expected = file_get_contents('data/CorporationSheetPublic.xml.aspx');
	...
	{
		$expected = file_get_contents('data/CorporationSheetPrivate.xml.aspx');
	...
	{
		$expected = file_get_contents('data/APIKeyInfo.xml.aspx');
	...
	{
		$expected = file_get_contents('data/CharacterSheet.xml.aspx');
	...
	{
		$expected = file_get_contents('data/TestResponse.xml.aspx');
		$request = $this->mockRequest();
		$params = array(
			'keyID' => 654321, 
			'vCode' => '0234567890123456789022345678903234567890423456789052345678906234',
	...
	/**
	 * @expectedException AleExceptionEVEMiscellaneous
	 */
	public function testErrorResponse()
	{
		$expected = file_get_contents('data/ErrorResponse.xml.aspx');
		$request = $this->mockRequest();
		$params = array(
			'keyID' => 123456, 
			'vCode' => '0234567890123456789022345678903234567890423456789052345678906234'
