...
require_once '../ale/cache/pdo.php';
...
class AleCachePdoTest extends PHPUnit_Framework_TestCase
{
	protected $pdo;
	...
	{
		if (file_exists('data/pdo_test.db')) {
			unlink('data/pdo_test.db');
		}
		copy('data/pdo.db', 'data/pdo_test.db');
		$this->pdo = new PDO('sqlite:data/pdo_test.db');
		$config = array();
		$config['db'] = $this->pdo;
		$this->cache = new AleCachePdo($config); 
